    ...
    for(ruch;ruch<10;ruch++)
{
    ...
    for(int jd=0;jd<3;jd++)
    {
        ...
        {cout<<"w "<<ruch<<" ruchach wygrywa krzyzyk";
        return 24;}
        if(T[0][2]==T[1][1] && T[1][1]==T[2][0] && T[1][1]=="x")
        {cout<<"w "<<ruch<<" ruchach wygrywa krzyzyk";
        return 24;
    }}
     for(int jd=0;jd<3;jd++)
    {
        ...
        {cout<<"w "<<ruch<<" ruchach wygrywa kolo";
        return 24;}
        if(T[0][2]==T[1][1] && T[1][1]==T[2][0] && T[1][1]=="o")
        {cout<<"w "<<ruch<<" ruchach wygrywa kolo";
        return 24;}
    }
    if(ruch%2==1)
    ...
    while(T[x][y]=="x" || T[x][y]=="o" || x>=3 || y>=3)
{
    cout<<"To pole jest juz zajete lub dane sa zle wpisane"<<endl;
    cout<<"Podaj ponownie wiersz"<<endl;
    cin>>x;
    cout<<"Podaj ponownie kolumne"<<endl;
    cin>>y;
    if((T[x][y]!="x" || T[x][y]!="o") && (x>=0 && x<3) && (y>=0 && y<3))
        break;
}
    if(ruch%2==1)
        T[x][y]="o";
